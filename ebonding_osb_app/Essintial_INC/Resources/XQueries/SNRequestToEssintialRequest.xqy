...
declare function local:func($StatusDestinationValue as xs:string, 
    ...
    return
       <ns2:serviceTicket sit:Version="1" sit:AcknowledgementNumber="{$MsgTranId}" sit:PreviousVORDID="{fn-bea:execute-sql('jdbc/ESBDevDB',xs:QName('EXTERNAL_REFNUM'),'SELECT EXTERNAL_REFNUM FROM EBONDINGS_TRANSACTIONS where TICKET_NUM=? ORDER BY UPDATE_TIME DESC OFFSET 0 ROWS FETCH FIRST 1 ROWS ONLY',fn:string($SNRequest/ns1:IncidentRequestBody/ns1:IncidentDetails/ns1:TicketNumber/text()))//text()}">
           <sit:LocationCode>{fn:substring(fn:concat(fn:data($InitialCreateRequest/ns1:IncidentRequestBody/ns1:IncidentLocation/ns1:Location),' ',fn:data($InitialCreateRequest/ns1:IncidentRequestBody/ns1:IncidentLocation/ns1:Street)),0,50)}</sit:LocationCode>
            ...
            <sit:Priority>{fn:data($InitialCreateRequest/ns1:IncidentRequestBody/ns1:IncidentDetails/ns1:Priority)}</sit:Priority>
            ...
            <sit:ReceivedMethod>{$MapData/ReceivedMethod}</sit:ReceivedMethod>
            <sit:CustomerReceivedDate>{$MapData/CustomerReceivedDate}</sit:CustomerReceivedDate>
            <sit:ScheduledDS>{$MapData/ScheduledDS}</sit:ScheduledDS>
            <sit:TechOnSite>{$MapData/TechOnSite}</sit:TechOnSite>
            <sit:TechCheckedOut>{$MapData/TechCheckedOut}</sit:TechCheckedOut>
            <sit:TotalLaborTime>{$MapData/TotalLaborTime}</sit:TotalLaborTime>
            <sit:TotalTravelTime>{$MapData/TotalTravelTime}</sit:TotalTravelTime>
        ...
        <ns2:serviceTicket sit:Version="1" sit:AcknowledgementNumber="{$MsgTranId}" sit:PreviousVORDID="{fn-bea:execute-sql('jdbc/ESBDevDB',xs:QName('EXTERNAL_REFNUM'),'SELECT EXTERNAL_REFNUM FROM EBONDINGS_TRANSACTIONS where TICKET_NUM=? ORDER BY UPDATE_TIME DESC OFFSET 1 ROWS FETCH FIRST 1 ROWS ONLY',fn:string($SNRequest/ns1:IncidentRequestBody/ns1:IncidentDetails/ns1:TicketNumber/text()))//text()}">
           <sit:LocationCode>{fn:substring(fn:concat(fn:data($SNRequest/ns1:IncidentRequestBody/ns1:IncidentLocation/ns1:Location),' ',fn:data($SNRequest/ns1:IncidentRequestBody/ns1:IncidentLocation/ns1:Street)),0,50)}</sit:LocationCode>
            ...
            <sit:Priority>{fn:data($SNRequest/ns1:IncidentRequestBody/ns1:IncidentDetails/ns1:Priority)}</sit:Priority>
            <sit:CustomerReference4>{fn:data($SNRequest/ns1:IncidentRequestHeader/ns1:RecType)}</sit:CustomerReference4>
            <sit:CustomerReference3>{fn:data($SNRequest/ns1:IncidentRequestBody/ns1:IncidentDetails/ns1:TicketNumber)}</sit:CustomerReference3>
            {
...
local:func($StatusDestinationValue, $ResolutionCodeDestinationValue,$ExternalRefNum,$MsgTranId,$ReopenFlag,$MapData, $SNRequest)